...
export default function MusicCard() {
	...
	const skipToNext = () => {
		const { currentIndex: idx, playMode: mode, musicList: list } = stateRef.current
		let nextIndex = idx
		if (mode === 'random') {
			nextIndex = Math.floor(Math.random() * list.length)
			// Avoid same song if possible
			if (nextIndex === idx && list.length > 1) {
	...
	const skipToPrevious = () => {
		const { currentIndex: idx, musicList: list } = stateRef.current
		let nextIndex = (idx - 1 + list.length) % list.length
		setCurrentIndex(nextIndex)
		setIsPlaying(true)
	}
