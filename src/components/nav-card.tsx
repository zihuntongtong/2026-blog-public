...
import { Home, ScrollText, Briefcase, Info, BookOpen, Globe, MessageSquare } from 'lucide-react'
...
const list = [
	{
		...
		iconActive: (props: any) => <Home {...props} fill="currentColor" />,
		label: '返回首页',
		href: '/'
	},
	{
		icon: (props: any) => <ScrollText {...props} />,
		iconActive: (props: any) => <ScrollText {...props} fill="currentColor" />,
		label: '我的文章',
		href: '/blog'
	},
	{
		icon: (props: any) => <Briefcase {...props} />,
		iconActive: (props: any) => <Briefcase {...props} fill="currentColor" />,
		label: '执业案例',
		href: '/projects'
	},
	{
		icon: (props: any) => <Info {...props} />,
		iconActive: (props: any) => <Info {...props} fill="currentColor" />,
		label: '关于网站',
		href: '/about'
	},
	{
		icon: (props: any) => <BookOpen {...props} />,
		iconActive: (props: any) => <BookOpen {...props} fill="currentColor" />,
		label: '书籍分享',
		href: '/share'
	},
	{
		icon: (props: any) => <Globe {...props} />,
		iconActive: (props: any) => <Globe {...props} fill="currentColor" />,
		label: '优秀网站',
		href: '/bloggers'
	},
	{
		icon: (props: any) => <MessageSquare {...props} />,
		iconActive: (props: any) => <MessageSquare {...props} fill="currentColor" />,
		label: '留言板',
		href: '/guestbook'
	}
]
			...
			<HomeDraggableLayer cardKey='navCard' x={position.x} y={position.y} width={styles.width} height={styles.height}>
				<Card
					...
					<Link className='flex items-center gap-3' href='/'>
						...
						{form === 'full' && <span className='text-brand mt-2 text-xs font-medium'>(深圳执业版)</span>}
					</Link>

					{(form === 'full' || form === 'icons') && (
						<>
							{form !== 'icons' && <div className='text-secondary mt-6 text-sm uppercase'>导航</div>}
							...
							<div className={cn('relative mt-2 space-y-2', form === 'icons' && 'mt-0 flex items-center gap-6 space-y-0')}>
								...
								{list
									.filter(item => !(form === 'full' && item.href === '/'))
									.map((item, index) => (
										<Link
											key={item.href}
											href={item.href}
